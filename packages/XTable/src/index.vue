<template>
  <div class="table-wrapper">
    <div class="table-area" :style="styleTable">
      <div :id="domInit.content" class="table-border table-content" :style="styleTable">
        <table class="fixed-table">
          ...
          <tfoot v-if="footerData && footerData.length > 0">
            <tr v-for="(rowItem, rowIndex) in footerData" :key="`tf-${config.key}-${rowItem[config.rowKey]}-${rowIndex}`">
              <template v-for="(columnItem, columnIndex) in tableColumns">
                <td
                  v-if="rowItem[columnItem.dataIndex].colSpan && rowItem[columnItem.dataIndex].rowSpan"
                  :colspan="rowItem[columnItem.dataIndex].colSpan > 1 ? rowItem[columnItem.dataIndex].colSpan : null"
...
export default {
  ...
  computed: {
    maxPage() {
      return Math.ceil(this.dataAmount / this.pageData.size);
    },
    dataAmount() {
    ...
    domInit() {
      return {
        content: `${this.config.key}-x-table-content`,
        left: `${this.config.key}-x-sticky_left_shadow`,
        right: `${this.config.key}-x-sticky_right_shadow`,
      };
    },
  },
  watch: {
    pageData: {
      handler() {
        console.log('pageData', this.maxPage, this.pageData);
        if (this.maxPage < this.pageData.page) {
          this.resetPage(this.maxPage);
        } else {
          this.getShowData();
        }
      },
  ...
  mounted() {
    ...
    const leftShadowDom = document.getElementById(this.domInit.left);
    const rightShadowDom = document.getElementById(this.domInit.right);
    if (leftShadowDom && rightShadowDom) {
      const contentDom = document.getElementById(this.domInit.content);
